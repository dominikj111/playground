...
impl Menu {
    pub fn construct<T: Component + Default>(
        ...
        let menu = RelativeLayout::new()
            .with_rel_1(Vec2::splat(-1.0))
            .with_rel_2(Vec2::splat(101.0))
            .build_as(tree, "Menu")?;

        let wiggle_amplitude = Vec2::new(2.4, 4.0);
        let background = WindowLayout::new()
            .size_rel((
                100.0 + wiggle_amplitude.x * 2.0,
                100.0 + wiggle_amplitude.y * 2.0,
            ))
            .build_in(tree, &menu)?;
        ...
        commands.spawn(ImageElementBundle::new(
            &board,
            ImageParams::default(),
            assets.main_board.clone(),
            Vec2::new(807.0, 1432.0),
        ));
        ...
        commands.spawn(ImageElementBundle::new(
            logo,
            ImageParams::default(),
            assets.main_logo.clone(),
            Vec2::new(1240.0, 381.0),
        ));
        ...
        // Build the grid
        let button_widget_list =
            segment
                .add_gaps(1.0)
                .build_in(tree, list, GridOrientation::Vertical)?;
        ...
        for i in 0..button_widget_list.len() {
            ...
            ui::MainMenuButton::new(button_widget_list[i].name()).construct(
                commands,
                assets,
                tree,
                button_widget_list[i].end(".Button"),
                button_components,
            )?;
...
mod script {
    ...
    pub(super) fn main_menu_button_actions(
        mut commands: Commands,
        assets: Res<MenuAssetCache>,
        mut trees: Query<&mut UiTree<MyData>>,
        mut query: Query<(&MainMenuButton, &lg::InputMouseClick), With<Widget>>,
    ...
    pub(super) fn wiggle_background_widget_animation<T: Component + Default>(
        mut trees: Query<&mut UiTree<T>>,
        mut query: Query<(&Widget, &mut WiggleBackgroundWidget)>,
    ) {
        for mut tree in &mut trees {
            for (widget, mut animation) in &mut query {
                ...
                if animation.degree.x >= TAU {
                    animation.degree.x -= TAU;
                }
                if animation.degree.y >= TAU {
                    animation.degree.y -= TAU;
                }
                ...
                window.pos_relative.x =
                    animation.degree.x.sin() * animation.amplitude.x - animation.amplitude.x;
                window.pos_relative.y =
                    animation.degree.y.sin() * animation.amplitude.y - animation.amplitude.y;
            }
        }
    }
}
